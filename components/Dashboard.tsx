...
const Dashboard: React.FC<DashboardProps> = ({ items, profile }) => {
  ...
  const caloriePercentage = Math.min(Math.round((totals.calories / profile.goalCalories) * 100), 100);

  const macroData = [
    { name: 'Protein', value: totals.protein, goal: profile.goalProtein, color: '#10b981' },
    { name: 'Carbs', value: totals.carbs, goal: profile.goalCarbs, color: '#3b82f6' },
    { name: 'Fats', value: totals.fats, goal: profile.goalFats, color: '#f59e0b' },
  ];
    ...
    <div className="pb-24 pt-6 px-4 space-y-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-gray-400">of {profile.goalCalories} kcal</p>
        </div>
